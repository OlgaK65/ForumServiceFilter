...
import telran.ashkelon2018.ForumService.service.AccountService;
...
public class AccountManagementController {
	...
	@DeleteMapping("/{login}")
	public UserProfileDto remove(@RequestBody UserRegDto userRegDto, 
			@RequestHeader("Authorization") String token) {
		return accountService.removeUser(null, token);
	}
	...
	public void changePassword(@RequestHeader("X-Authorization") String password,
			@RequestHeader("Authorization") String token){
		accountService.changePassworf(password, token);
	}	
}
